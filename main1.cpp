...
const char* myScene = "sponza.obj"; //這裡是load 3D檔的地方
...
// adjust view point (調整視角)
vec3 eye = vec3(20.0f, 5.0f, 0.0f);
vec3 center = vec3(1.0, 1.0, 0.0);
vec3 up = vec3(0.0, 100.0, 0.0);
...
void My_Keyboard(unsigned char key, int x, int y)
{
	...
	{
		cout << "W is press." << endl;
		eye = eye - vec3(0.1,0.1,0.1);
		//center = center + vec3((center.x - eye.x) * 0.02, (center.y - eye.y) * 0.02, (center.z - eye.z) * 0.02);
		view = lookAt(eye, center, up);
	...
	else if (key == 's')
	{
		...
		cout << "Q is press." << endl;
		eye = vec3(eye[0],eye[1]-0.1,eye[2]);
		center = vec3(center[0],center[1]-1,center[2]);
		view = lookAt(eye, center, up);
		cout << eye[0] << eye[1] << eye[2] << endl;
		cout << center[0] << center[1] << center[2] << endl;
	}
	else if (key == 'e') {
		cout << "E is press." << endl;
		eye = vec3(eye[0], eye[1] + 0.1, eye[2]);
		center = vec3(center[0], center[1] + 1, center[2]);
		view = lookAt(eye, center, up);
		cout << eye[0] << eye[1] << eye[2] << endl;
